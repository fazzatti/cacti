...
import { AssetBridgingApp, IAssetBridgingApp } from "./asset-bridging-app";
...
export async function launchApp(
  ...
  if (
    ...
    process.env.STELLAR_ADMIN_SK == undefined ||
    process.env.STELLAR_CHAIN == undefined ||
    process.env.STELLAR_BRIDGE_PK == undefined
  ) {
    throw new Error("Env variables not set");
  }

  const appOptions: IAssetBridgingApp = {
    apiHost: process.env.API_HOST,
    apiServer1Port: parseInt(process.env.API_SERVER_1_PORT),
    apiServer2Port: parseInt(process.env.API_SERVER_2_PORT),
    stellarAdminSk: process.env.STELLAR_ADMIN_SK as string,
    stellarChain: process.env.STELLAR_CHAIN as SupportedStellarNetworkChain,
    stellarBridgePk: process.env.STELLAR_BRIDGE_PK,
    clientGatewayKeyPair: clientGatewayKeyPair,
    serverGatewayKeyPair: serverGatewayKeyPair,
    logLevel: "DEBUG",
  };

  const assetBridgingApp = new AssetBridgingApp(appOptions);
  try {
    await assetBridgingApp.start();
    console.info("AssetBridgingApp running...");
  } catch (ex) {
    console.error(`AssetBridgingApp crashed. Existing...`, ex);
    await assetBridgingApp.stop();
    process.exit(-1);
  }
}
