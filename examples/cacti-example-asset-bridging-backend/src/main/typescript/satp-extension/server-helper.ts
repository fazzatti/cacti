...
import { StellarSatpGateway } from "./stellar-satp-gateway";
...
export class ServerHelper extends ServerGatewayHelper {
  ...
  async checkValidInitializationRequest(
    ...
    if (gateway instanceof StellarSatpGateway) {
      await gateway
        .isValidBridgeBackCBDC(
          request.payloadProfile.assetProfile.keyInformationLink[1].toString(), // StellarID
