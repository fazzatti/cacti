...
export class StellarSatpGateway extends PluginSatpGateway {
  ...
  public setContracts(
    ...
    this.assetReferenceContractId = assetReferenceContractId;
  }
  ...
  async isValidBridgeBackCBDC(stellarID: string, stellarAddress: string) {
    // we should verify if the CBDC being bridged is valid or not
    // e.g. if a client is trying to send CBDC to another client in the sidechain, this should fail

    // FIXME : Not validating account addresses pairs.
    return true;
  }
  ...
  async createAssetToRollback(
    ...
    if (this.stellarApi != undefined) {
      const response = await this.stellarApi.runSorobanTransactionV1({
        ...
        method: "refund",
        methodArgs: {
          amount: sessionData.assetProfile.keyInformationLink[0],
          account: sessionData.assetProfile.keyInformationLink[1].toString(),
        },
        transactionInvocation: this.defaultTxInvocation(),
      } as StellarRunTransactionRequest);
  ...
  async createAsset(sessionID: string, assetId?: string): Promise<string> {
    ...
    if (this.stellarApi != undefined) {
      this.log.debug("SSESSIONDATA:", JSON.stringify(sessionData));
      ...
      sessionData.commitAcknowledgementClaim = stellarCreateAssetProof;

      this.sessions.set(sessionID, sessionData);

      this.log.info(
        `${fnTag}, proof of the asset create: ${stellarCreateAssetProof}`,
      );
