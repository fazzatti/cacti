...
import { getEthAddress, assertEqual, getStellarPk } from "./common";
...
Then(
  ...
  async function (
    ...
    const stellarId = getStellarPk(finalUser);
    ...
    const assetProfile = {
      ...
      // [amountBeingTransferred, stellarId, ethAddress]
...
Then(
  ...
  async function (user: string, amount: number, assetRefID: string) {
    const address = getEthAddress(user);
    const stellarPk = getStellarPk(user);
